...
<img src="/post/Auto_Np/Auto_Np_files/figure-html/unnamed-chunk-6-1.png" width="1344" />
...
<img src="/post/Auto_Np/Auto_Np_files/figure-html/unnamed-chunk-7-1.png" width="864" />
...
<img src="/post/Auto_Np/Auto_Np_files/figure-html/unnamed-chunk-8-1.png" width="1344" />
...
```
    ...
    ## # A tibble: 10 × 5
    ##     序号 Model            Engine  Explain            可调参数数量
    ##    <dbl> <chr>            <chr>   <chr>                     <dbl>
    ##  1     1 discrim_flexible earth   多项式判别法                  3
    ##  2     2 naive_Bayes      klaR    朴素贝叶斯                    2
    ##  3     3 nearest_neighbor kknn    KNN最近邻                     3
    ##  4     4 decision_tree    rpart   决策树                        3
    ##  5     5 rand_forest      ranger  随机森林                      3
    ##  6     6 boost_tree       xgboost 增强树                        8
    ##  7     7 svm_rbf          kernlab 径向基支持向量机              3
    ##  8     8 svm_poly         kernlab 多项式基支持向量机            4
    ##  9     9 mlp              nnet    神经网络                      3
    ## 10    10 multinom_reg     nnet    多分类逻辑回归                1
...
```
    ...
    ## Splitrule:                        gini 
    ## OOB prediction error (Brier s.):  0.09169374
...
<img src=" /pic/set_engine.png" width="1050" />
...
```
    ...
    ##               Model  Engine  accuracy     kappa   roc_auc            Explain
    ## 1          svm_poly kernlab 0.9795918 0.9639175 0.9902211 多项式基支持向量机
    ## 2       naive_Bayes    klaR 0.9387755 0.8906250 0.9753401         朴素贝叶斯
    ## 3        boost_tree xgboost 0.9183673 0.8518519 0.9502551             增强树
    ## 4  nearest_neighbor    kknn 0.8979592 0.8157895 0.9761905          KNN最近邻
    ## 5       rand_forest  ranger 0.8979592 0.8138298 0.9698129           随机森林
    ## 6           svm_rbf kernlab 0.8979592 0.8138298 0.9876701   径向基支持向量机
    ## 7  discrim_flexible   earth 0.8775510 0.7717391 0.9621599       多项式判别法
    ## 8     decision_tree   rpart 0.8571429 0.7512690 0.9370748             决策树
    ## 9               mlp    nnet 0.8571429 0.7393617 0.9566327           神经网络
    ## 10     multinom_reg    nnet 0.8367347 0.7241379 0.9124150     多分类逻辑回归
...
```
    ...
    ##               Model  Engine  accuracy     kappa   roc_auc            Explain
    ## 1          svm_poly kernlab 0.9795918 0.9639175 0.9902211 多项式基支持向量机
    ## 2           svm_rbf kernlab 0.8979592 0.8138298 0.9876701   径向基支持向量机
    ## 3  nearest_neighbor    kknn 0.8979592 0.8157895 0.9761905          KNN最近邻
    ## 4       naive_Bayes    klaR 0.9387755 0.8906250 0.9753401         朴素贝叶斯
    ## 5       rand_forest  ranger 0.8979592 0.8138298 0.9698129           随机森林
    ## 6  discrim_flexible   earth 0.8775510 0.7717391 0.9621599       多项式判别法
    ## 7               mlp    nnet 0.8571429 0.7393617 0.9566327           神经网络
    ## 8        boost_tree xgboost 0.9183673 0.8518519 0.9502551             增强树
    ## 9     decision_tree   rpart 0.8571429 0.7512690 0.9370748             决策树
    ## 10     multinom_reg    nnet 0.8367347 0.7241379 0.9124150     多分类逻辑回归
...
<img src="/post/Auto_Np/Auto_Np_files/figure-html/unnamed-chunk-32-1.png" width="960" />
...
<img src="/post/Auto_Np/Auto_Np_files/figure-html/unnamed-chunk-35-1.png" width="672" />
...
<img src="/post/Auto_Np/Auto_Np_files/figure-html/unnamed-chunk-36-1.png" width="672" />
...
```
    ...
    ## [1] "保险风险等级+品牌+燃料类型+增压机类型+车门数+车身风格+驱动轮+发动机位置+轴距+长度+宽度+高度+整车重量+发动机类型+气缸数量+发动机尺寸+燃油系统+缸径+冲程+压缩比+马力+最大转速+城市路百公里油耗"
...
```
    ...
    ## 
    ## Formula:
    ## 价格 ~ 保险风险等级 + 品牌 + 燃料类型 + 增压机类型 + 
    ##     车门数 + 车身风格 + 驱动轮 + 发动机位置 + 
    ##     轴距 + 长度 + 宽度 + 高度 + 整车重量 + 发动机类型 + 
    ##     气缸数量 + 发动机尺寸 + 燃油系统 + 缸径 + 
    ##     冲程 + 压缩比 + 马力 + 最大转速 + 城市路百公里油耗
    ## Total model degrees of freedom 54 
    ## 
    ## GCV score: 4941821     rank: 54/57
...
<img src="/post/Auto_Np/Auto_Np_files/figure-html/unnamed-chunk-39-1.png" width="672" />
